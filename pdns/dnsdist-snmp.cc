...
{
std::unique_ptr<DNSDistSNMPAgent> DNSDistSNMPConfig::s_snmpAgent{nullptr};
bool DNSDistSNMPConfig::s_snmpTrapsEnabled{false};

void DNSDistSNMPConfig::enable(const std::string& name, const std::string& daemonSocket)
{
  if (!isEnabled()) {
    s_snmpAgent = std::make_unique<DNSDistSNMPAgent>(name, daemonSocket);
  }
}

void DNSDistSNMPConfig::start()
{
  if (isEnabled()) {
    s_snmpAgent->run();
  }
