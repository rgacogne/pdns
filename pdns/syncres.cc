...
bool SyncRes::doResolveAtThisIP(const std::string& prefix, const DNSName& qname, const QType& qtype, LWResult& lwr, boost::optional<Netmask>& ednsmask, const DNSName& auth, bool const sendRDQuery, const DNSName& nsName, const ComboAddress& remoteIP, bool doTCP, bool* truncated)
{
  ...
  if(resolveret != 1) {
    ...
    if(resolveret != -2) { // don't account for resource limits, they are our own fault
      t_nsSpeeds[nsName].submit(remoteIP, 1000000, &d_now); // 1 sec
      ...
      if (s_serverdownmaxfails > 0 && (auth != g_rootdnsname) && t_fails.incr(remoteIP) >= s_serverdownmaxfails) {
        ...
        // mark server as down
        t_throttle.throttle(d_now.tv_sec, boost::make_tuple(remoteIP, "", 0), s_serverdownthrottletime, 10000);
      }
      else if (resolveret == -1) {
        // unreachable, 1 minute or 100 queries
        t_throttle.throttle(d_now.tv_sec, boost::make_tuple(remoteIP, qname, qtype.getCode()), 60, 100);
      }
      else {
        // timeout
        t_throttle.throttle(d_now.tv_sec, boost::make_tuple(remoteIP, qname, qtype.getCode()), 10, 5);
      }
    }
  ...
  if(lwr.d_rcode==RCode::ServFail || lwr.d_rcode==RCode::Refused) {
    ...
    t_throttle.throttle(d_now.tv_sec, boost::make_tuple(remoteIP, qname, qtype.getCode()), 60, 3);
    return false;
  }
