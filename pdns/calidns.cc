...
try
{
  ...
  desc.add_options()
    ...
    ("plot-file", po::value<string>(), "Write results to this file")
    ("quiet", "Whether to run quietly, writing only the maximum QPS reached. This option only makes when used with minimum-success-rate")
  ...
  for(qps=qpsstart;;) {
    ...
    if (minimumSuccessRate > 0.0 && perc < minimumSuccessRate) {
      if (beQuiet) {
        ...
        cout<<"The final QPS rate reached before failing was: "<<bestQPS<<endl;
      }
      break;
    }
