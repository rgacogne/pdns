...
{
  return new uint64_t(t_RC->doDump(fd) + dumpNegCache(SyncRes::t_negcache, fd) + t_packetCache->doDump(fd));
}
...
{
  uint64_t ret = SyncRes::t_negcache.wipe(canon, subtree);
...
{
  uint64_t tmp=(SyncRes::t_negcache.size());
