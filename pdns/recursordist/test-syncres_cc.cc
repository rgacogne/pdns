...
static void initSR(std::unique_ptr<SyncRes>& sr, bool edns0, bool dnssec, SyncRes::LogMode lm=SyncRes::LogNone, time_t fakeNow=0)
{
  ...
  SyncRes::setAuthAndForwardDomainsMap(std::make_shared<SyncRes::AuthAndForwardDomainsMap_t>());
}
...
BOOST_AUTO_TEST_CASE(test_root_nx_trust) {
  ...
  BOOST_CHECK_EQUAL(SyncRes::t_negcache.size(), 2);
...
BOOST_AUTO_TEST_CASE(test_root_nx_dont_trust) {
  ...
  BOOST_CHECK_EQUAL(ret.size(), 1);
  /* one for target1 */
  BOOST_CHECK_EQUAL(SyncRes::t_negcache.size(), 1);
...
BOOST_AUTO_TEST_CASE(test_skip_negcache_for_variable_response) {
  ...
  BOOST_CHECK_EQUAL(SyncRes::t_negcache.size(), 0);
}
...
BOOST_AUTO_TEST_CASE(test_forward_zone_nord) {
  ...
  fd.d_rd = false;
  fd.d_name = target;
  fd.d_servers.push_back(forwardedNS);
  auto map = std::make_shared<SyncRes::AuthAndForwardDomainsMap_t>();
  map->add(target, fd);
  SyncRes::setAuthAndForwardDomainsMap(map);
