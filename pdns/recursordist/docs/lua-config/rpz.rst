...
Note that since 4.2.0, the override policy specified via 'defpol' is no longer applied to local data entries present in the zone by default, but this behavior can be changed via the 'defpolOverrideLocalData' parameter.
...
^^^^^^^^^^^^^^^^^^^^^^^
.. versionadded:: 4.2.0
  Before 4.2.0 local data entries were also overridden by the default policy.

Whether local data entries should be overridden by the default policy. Default is false.
