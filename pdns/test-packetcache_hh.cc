...
BOOST_AUTO_TEST_CASE(test_PacketCacheAuthCollision) {
...

  {
    ...
    auto hash1 = PacketCache::canHashPacket(spacket1, false, 0);
    ...
    auto hash2 = PacketCache::canHashPacket(spacket2, false, 0);
    ...
    for (size_t idxA = 0; idxA < 256; idxA++) {
      for (size_t idxB = 0; idxB < 256; idxB++) {
        for (size_t idxC = 0; idxC < 256; idxC++) {
          ...
          pwFQ.commit();
          auto secondKey = PacketCache::canHashPacket(std::string(reinterpret_cast<const char *>(secondQuery.data()), secondQuery.size()), false, 0);
    ...
    for (size_t idxA = 0; idxA < 256; idxA++) {
      for (size_t idxB = 0; idxB < 256; idxB++) {
        for (size_t idxC = 0; idxC < 256; idxC++) {
          ...
          pwSQ.commit();
          secondKey = PacketCache::canHashPacket(std::string(reinterpret_cast<const char *>(secondQuery.data()), secondQuery.size()), false, 0);
...
BOOST_AUTO_TEST_CASE(test_PacketCacheRecSimple) {
...

  {
    ...
    *(ptr + 1) = 255;
    ...
    BOOST_CHECK_NO_THROW(PacketCache::canHashPacket(spacket1, true, 0));
  }
}
...
BOOST_AUTO_TEST_CASE(test_PacketCacheRecCollision) {
...

  {
    ...
    pw1.getHeader()->id = 0x42;
    string spacket1((const char*)&packet[0], packet.size());
    auto hash1 = PacketCache::canHashPacket(spacket1, true, 0);
    ...
    pw2.getHeader()->id = 0x84;
    string spacket2((const char*)&packet[0], packet.size());
    auto hash2 = PacketCache::canHashPacket(spacket2, true, 0);
