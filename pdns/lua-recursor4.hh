...
public:
  ...
  struct PolicyHitEvent
  {
    PolicyHitEvent(const ComboAddress& rem, const DNSName& name, const QType& type, bool tcp): qname(name), qtype(type), remote(rem), isTcp(tcp)
    {
    }
    const DNSName& qname;
    const QType qtype;
  ...
  bool policyHitFilter(const ComboAddress& remote, const DNSName& qname, const QType& qtype, bool tcp, DNSFilterEngine::Policy& policy, std::unordered_set<std::string>& tags, std::unordered_map<std::string,bool>& dicardedPolicies) const;
...
private:
  ...
  typedef std::function<bool(PolicyHitEvent&)> policyEventFilter_t;
  policyEventFilter_t d_policyEventFilter;
};
