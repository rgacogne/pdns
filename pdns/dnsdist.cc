    ...
    try {
      ...
      for (const auto& fd : sockets) {
        ...
        */
        ...
        if (!responseContentMatches(response, ids->qname, ids->qtype, ids->qclass, dss->d_config.remote, qnameWireLength)) {
          continue;
        }
        ...
        if(IDState::isInUse(usageIndicator) && ids.age++ > g_udpTimeout) {
          ...
          */
          ...
          vinfolog("Had a downstream timeout from %s (%s) for query for %s|%s from %s",
                   dss->d_config.remote.toStringWithPort(), dss->getName(),
          ...
          g_rings.insertResponse(ts, ids.origRemote, ids.qname, ids.qtype, std::numeric_limits<unsigned int>::max(), 0, fake, dss->d_config.remote, dss->getProtocol());
        }
      }
    }
