...
static void handleResponseForUDPClient(IDState& ids, PacketBuffer& response, uint16_t maxPayloadSize, LocalStateHolder<vector<DNSDistResponseRuleAction>>& localRespRuleActions, const std::shared_ptr<DownstreamState>& ds, bool selfGenerated, std::optional<uint16_t> queryId)
{
    ...
    try {
      ...
      for (const auto& fd : sockets) {
        ...
        */
        ...
        handleResponseForUDPClient(*ids, response, 0, localRespRuleActions, dss, false, queryId);
      }
    }
    catch (const std::exception& e){
...
public:
  ...
  void handleResponse(const struct timeval& now, TCPResponse&& response) override
  {
    ...
    handleResponseForUDPClient(ids, response.d_buffer, d_payloadSize, localRespRuleActions, d_ds, response.d_selfGenerated, std::nullopt);
  }
