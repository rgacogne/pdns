...
static void handleResponseForUDPClient(InternalQueryState& ids, PacketBuffer& response, uint16_t maxPayloadSize, LocalStateHolder<vector<DNSDistResponseRuleAction>>& localRespRuleActions, const std::shared_ptr<DownstreamState>& ds, bool selfGenerated, std::optional<uint16_t> queryId)
{
    ...
    try {
      ...
      for (const auto& fd : sockets) {
        ...
        */
        ...
        handleResponseForUDPClient(ids->internal, response, 0, localRespRuleActions, dss, false, queryId);
      }
    }
    catch (const std::exception& e) {
