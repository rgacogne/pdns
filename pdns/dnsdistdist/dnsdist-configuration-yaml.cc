...
static bool handleTLSConfiguration(const Context& context, const dnsdist::rust::settings::BindConfiguration& bind, ClientState& state, const std::shared_ptr<const TLSFrontend>& parent)
{
  ...
  if (protocol == "dot") {
    ...
    if (frontend->d_library == "h2o") {
      ...
      SLOG(errlog("DOH bind %s is configured to use libh2o but the library is not available", bind.listen_address),
           context.logger->error(Logr::Error, "DoH frontend is configured to use libh2o but the library is not available", "frontend.address", Logging::Loggable(bind.listen_address)));
      return false;
