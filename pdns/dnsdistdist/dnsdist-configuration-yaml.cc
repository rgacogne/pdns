  ...
  if (protocol == "dot") {
    ...
    if (frontend->d_library == "h2o") {
      ...
      SLOG(errlog("DOH bind %s is configured to use libh2o but the library is not available", bind.listen_address),
           context.logger->error(Logr::Error, "DoH frontend is configured to use libh2o but the library is not available", "listen-address", Logging::Loggable(bind.listen_address)));
      return false;
