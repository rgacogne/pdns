...
BOOST_AUTO_TEST_CASE(test_LMDB) {
  ...
  string dbPath("/tmp/test_lmdb.XXXXXX");
  {
    ...
    transaction->put(dbi, MDBInVal(ids.qname.toDNSStringLC()), MDBInVal("this is the value for the qname"));
...
BOOST_AUTO_TEST_CASE(test_CDB) {
  ...
  char db[] = "/tmp/test_cdb.XXXXXX";
  {
    ...
    BOOST_REQUIRE(writer.addEntry(ids.qname.toDNSStringLC(), "this is the value for the qname"));
