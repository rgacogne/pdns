...
public:
  ...
  void handleResponse([[maybe_unused]] const struct timeval& now, TCPResponse&& response) override
  {
    ...
    if (!unit->ids.selfGenerated) {
      ...
      VERBOSESLOG(infolog("Got answer from %s, relayed to %s (DoH3, %d bytes), took %f us", unit->downstream->d_config.remote.toStringWithPort(), unit->ids.origRemote.toStringWithPort(), unit->response.size(), udiff),
